...
def create_cert(cert_file):
    ...
    cmd = ["openssl", "req -x509 -nodes -days 365 -newkey rsa:2048 -keyout %s -out %s" %
                      cert_file]
    subprocess.call([cmd])
...
def verify_file(certfile, sigfd):
    ...
    if p.returncode == 4:
        ...
        JSONErrorVar(('Signature does not match the data.','',1), stdoutdata, stderrdata)
    elif p.returncode > 0:  # Unknown error
        JSONErrorVar(('Error with code %%s.',p.returncode,1), stdoutdata, stderrdata)
